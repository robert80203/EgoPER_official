...
class VideoDataSet(data.Dataset):
    ...
    def _get_range_indices(self, record):
        max_frame_idx = max(1, record.num_frames - self.num_consecutive_frames + 1)
        return [i+1 for i in range(max_frame_idx)]

    def _get_feature_indices(self, record):
        max_frame_idx = max(1, record.num_frames - self.num_consecutive_frames + 1)
        return [i+1 for i in range(max_frame_idx)]
        if self.dense_sampling:
            if self.fixed_offset:
                ...
                t_stride = self.sample_freq
                #start_list = np.linspace(0, sample_pos - 1, num=self.num_clips, dtype=int)
                start_list = np.arange(0, sample_pos - 1,dtype=int)
                frame_idx = []
                for start_idx in start_list.tolist():
                    frame_idx += [(idx * t_stride + start_idx) % max_frame_idx for idx in range(self.num_frames)]
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            indices = self._get_range_indices(record)
        ...
        images = self.transform(images) # 3 x n_frames x h x w
        if self.test_mode:
        ...
        return images, label, record.video_id
