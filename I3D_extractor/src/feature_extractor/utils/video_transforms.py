...
class GroupScale(object):
    ...
    change PIL.Image resize to cv2.resize
    """
    ...
    def __call__(self, img_group):
        ...
        for img in img_group:
            ...
            h, w = arr.shape[:2]
            if h <= w:
                resize_h, resize_w = self.size, int(self.size * w / h)
            else:
                resize_h, resize_w = int(self.size * h / w), self.size
            cv_down = cv2.resize(arr, (resize_w, resize_h), fx=0, fy=0, interpolation=self.interpolation)
            cv_down = Image.fromarray(cv_down)
            imgs.append(cv_down)
        return imgs
