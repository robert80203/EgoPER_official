...
def parse_excel(excel):
    ...
    for i, row in excel.iterrows():
        ...
        if action == 'start':
            ...
            continue
        if action == 'end':
            end_time = row['start time (sec)']
            reached_end = True
            continue
        if 'narration' in action.lower():
            narration_level = row['start time (sec)']
            continue
        ...
        if not isinstance(note, float):
            if note[0] == 'E':
                ...
                assert note[2] == ':' or note[2] == '.', note[2]
                ec = ( 'Error', int(note[1]) )
...
xl = pd.ExcelFile('/mnt/raptor/datasets/PTG_dataset/recording/batch4/Data_Annotation.xlsx')
labels = {}
...
nframes = {}
for sheet_name in xl.sheet_names[1:]:    
    ffolder = f'/mnt/raptor/datasets/PTG_dataset/recording/batch4/frames_fps{FPS}/{sheet_name}/'
...
for vname, label in labels.items():
    ...
    with open(f"/mnt/raptor/datasets/PTG_dataset/recording/batch4/processed/label_v1/groundTruth_fps10/{vname}.txt", "w") as fp:
