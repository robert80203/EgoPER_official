...
def make_optimizer(model, optimizer_config):
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv2d, torch.nn.Conv1d, MaskedConv1D, torch.nn.Embedding)
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm, torch.nn.BatchNorm1d, torch.nn.InstanceNorm1d, torch.nn.Dropout, torch.nn.Dropout2d)
...
def train_one_epoch_diffact(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        if use_contrastive:
            ...
            for iter_c_idx, video_c_list in enumerate(train_loader2):
                if num_videos <= max_videos * len(video_list):
                    model(video_c_list, mode='clustering')
...
def valid_one_epoch_diffact(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        # labels = video_list[0]['labels']
        # segments = video_list[0]['segments']
        ...
        with torch.no_grad():
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['segments'].shape[0] > 0:
                    ...
                    # 200 results
                    #print(output[vid_idx]['labels'], output[vid_idx]['segments'])
                    
                    #print(len(results['video-id']), output[vid_idx]['segments'].size(), output[vid_idx]['labels'].size())
