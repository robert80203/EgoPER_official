    ...
    if scores is None:
        ...
        for j in indices:
            if segments[j, 1] != segments[j, 0]: #and scores[j] > 0.3:
                #print(scores[j], labels[j])
                preds[int(segments[j, 0])*fps:int(segments[j, 1])*fps] = labels[j]
                #print(int(segments[j, 0]), int(segments[j, 1]), labels[j], scores[j])
        # for j in error_indices:
        #     preds[int(segments[j, 0])*fps:int(segments[j, 1])*fps] = error_idx

    return preds.long()


def precision_recall_f1(all_preds, all_gts, set_labels, bg_to_normal = None):
...
def get_gt(dataset_name, action2idx, error_label_dict):
    
    if 'pinwheels' in dataset_name:
        video_list =  [5, 15, 25]
        label_path = '/mnt/raptor/datasets/PTG_dataset/pinwheel_recording/processed/label_v1/groundTruth_fps10/'
        video_prefix = 'Video'
    elif 'make_tea' in dataset_name:
        video_list =  [5, 15, 25]
        label_path = '/mnt/raptor/datasets/PTG_dataset/three_recipes/processed/groundTruth_10fps/'
        video_prefix = 'make_tea_'
    elif 'quesadilla' in dataset_name:
        video_list =  [5, 15, 25]
        label_path = '/mnt/raptor/datasets/PTG_dataset/three_recipes/processed/groundTruth_10fps/'
        video_prefix = 'quasadellia_'

    labels = []
    lengths_id = []
    for video_id in video_list:
        with open(label_path + video_prefix + str(video_id) + '.txt', 'r') as fp:
            label_list = fp.readlines()
            for line in label_list:
                tokens = line.split('|')
                action = action2idx[tokens[0].strip('\n')]
                labels.append(action)
        lengths_id.append([len(label_list), video_id])
    return np.array(labels, dtype=np.int16), lengths_id

def evaluate_action_segmentation_cls_head(output_file, dataset_name, action2idx, error_label_dict, set_labels):
    
    with open(output_file, "rb") as f:
        results = pickle.load(f)

    gt_labels, video_lengths_id = get_gt(dataset_name, action2idx, error_label_dict)

    t_start = torch.tensor(results['t-start']).unsqueeze(1)
    t_end = torch.tensor(results['t-end']).unsqueeze(1)
    segments = torch.cat((t_start, t_end), dim=1)
    labels = torch.tensor(results['label'])
    scores = torch.tensor(results['score'])
    video_list = torch.tensor([int(i) for i in results['video-id']])

    all_preds = None
    fps = 10
    for i in range(len(video_lengths_id)):
        length, video_id = video_lengths_id[i]
        idx_list = video_list == video_id
        all_pred = to_frame_wise(segments[idx_list], labels[idx_list], scores[idx_list], length, fps=fps)

        if all_preds is None:
            all_preds = all_pred
        else:
            all_preds = torch.cat((all_preds, all_pred), dim=0)
    
    p, r, f1 = precision_recall_f1(all_preds, gt_labels, set_labels, bg_to_normal=None)
    return f1
